...
class ThirdYearControllerTest extends WebTestCase
{
    private KernelBrowser $client;
    private EntityManagerInterface $em;
    private string $token = '';

    private const FIRST_SECTION_IDS = [1, 2];
    private const SECOND_SECTION_IDS = [3, 4];
    private const THIRD_SECTION_IDS = [5, 6];
    ...
    private function createSectionFaculties(): void
    {
        ...
        foreach (self::THIRD_SECTION_IDS as $id) {
            $connection->executeStatement(
                "INSERT IGNORE INTO faculty (id, name) VALUES (:id, :name)",
                ['id' => $id, 'name' => 'Third Section Faculty ' . $id]
            );
        }
    ...
    public function testGetThirdYearSubjectsByGroupSuccess(): void
    {
        ...
        $this->client->request(
            'GET',
            '/admin/group-subject-teacher/subjects/third-year/by-group/' . $group->getId(),
            [],
            [],
            [
    ...
    public function testGetThirdYearSubjectsByGroupNotFound(): void
    {
        $nonExistentId = 99999;

        $this->client->request(
            'GET',
            '/admin/group-subject-teacher/subjects/third-year/by-group/' . $nonExistentId,
            [],
            [],
            [
                'HTTP_AUTHORIZATION' => 'Bearer ' . $this->token,
    ...
    public function testCreateThirdYearSuccess(): void
    {
        $faculty = $this->getFirstSectionFaculty();
        ...
        $course = $this->getOrCreateCourse(3);
        $group = $this->createGroup($course, $faculty);
        $subject = $this->createSubject($faculty, 3);

        $this->client->request(
            'POST',
            '/admin/group-subject-teacher/third-year',
            [],
            [],
            [
                'HTTP_AUTHORIZATION' => 'Bearer ' . $this->token,
        ...
        $this->assertEquals("3-kurs uchun to'g'ri biriktirildi", $data['message']);
    ...
    public function testCreateThirdYearMissingParameters(): void
    {
        $this->client->request(
            'POST',
            '/admin/group-subject-teacher/third-year',
    ...
    public function testCreateThirdYearNotFound(): void
    {
        $this->client->request(
            'POST',
            '/admin/group-subject-teacher/third-year',
    ...
    public function testCreateThirdYearDuplicateGroupSubject(): void
    {
        $faculty = $this->getFirstSectionFaculty();
        $course = $this->getOrCreateCourse(3);
        $group = $this->createGroup($course, $faculty);
    ...
    public function testCreateThirdYearNotThirdYearGroup(): void
    {
        $faculty = $this->getFirstSectionFaculty();
        ...
        $data = json_decode($response->getContent(), true);
        $this->assertArrayHasKey('error', $data);
        $this->assertStringContainsString("3-kurslar", $data['error']);
    }

    public function testCreateThirdYearTeacherWorkloadExceeded(): void
    {
        $this->markTestSkipped('Teacher workload logic tested in Service layer');
    }

    public function testCreateThirdYearInvalidFacultySection(): void
    {
        ...
        $subject = $this->createSubject($faculty1, 3);
    ...
    public function testCreateThirdYearValidFacultySection(): void
    {
        $faculty1 = $this->getFirstSectionFaculty();
    ...
    public function testCreateThirdYearFirstToSecondSectionInvalid(): void
    {
        $faculty1 = $this->getFirstSectionFaculty();
        $faculty2 = $this->getSecondSectionFaculty();
        ...
        $this->assertEquals(
            Response::HTTP_BAD_REQUEST,
            $response->getStatusCode(),
            "Should fail when teacher and subject are in different sections"
        );
    }

    public function testCreateThirdYearSecondToThirdSectionInvalid(): void
    {
        $faculty1 = $this->getSecondSectionFaculty();
        $faculty2 = $this->getThirdSectionFaculty();
    ...
    public function testCreateThirdYearThirdSectionValid(): void
    {
        $faculty1 = $this->getThirdSectionFaculty();
        ...
        $this->assertEquals(
            Response::HTTP_OK,
            $response->getStatusCode(),
            "Should succeed within third section"
        );
    }
    ...
    private function getSecondSectionFaculty(): Faculty
    {
        ...
        return $this->createFacultyWithId(3);
    }

    private function getThirdSectionFaculty(): Faculty
    {
        foreach (self::THIRD_SECTION_IDS as $id) {
            $faculty = $this->em->getRepository(Faculty::class)->find($id);
            if ($faculty) {
        ...
        return $this->createFacultyWithId(5);
    }
